...
formulaire.addEventListener("keyup", function (e) {
  ...
  if (input_focus === "pseudo") {
    ...
    const input_email_recuperation = document.querySelector(
      "input[type='email']"
    );
...
function open_modal_mot_de_passe_oublier(instruction) {
  const modal_mot_de_passe_oublier = document.querySelector(
    ".modal_mot_de_passe_oublier"
  );
...
function send_email_de_recuperation() {
  if (input_email_recuperation.className !== "error") {
    ...
    xhr.open(
      "POST",
      baseUrl + "controllers/MotDePasseOublierController.php",
      true
    );
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function () {
