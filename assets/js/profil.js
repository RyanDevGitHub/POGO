...
formulaire.addEventListener("keyup", function (e) {
  ...
  let form = document.forms["form_profil"];
  if (input_focus === "pseudo") {
    ...
    xhr.open("POST", baseUrl + "controllers/CheckEmailController.php", true);
    ...
    if ((value.length === 5 && value.match(numbers)) || value.length === 0) {
      ...
      xhr.open("POST", baseUrl + "controllers/SearchCityController.php", true);
      xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
      xhr.onreadystatechange = function () {
        if (this.readyState == 4 && this.status === 200) {
    ...
    var xhr = new XMLHttpRequest();
    xhr.open("POST", baseUrl + "controllers/CheckMdpController.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function () {
      if (this.readyState == 4 && this.status === 200) {
        response = xhr.responseText;
        ...
        if (response === "fail_mdp") {
          input.classList.add("error");
          input.setCustomValidity(
            "Votre mot de passe est invalide il doit contenir 1MAJ 1NOMBRE 1SYMBOLE"
          );
          if (value.length === 0) {
            input.classList.remove("error");
            input.setCustomValidity("");
...
function addClassMonCompte() {
  ...
  const historique_de_commande = document.querySelector(
    ".historique_de_commande"
  );
  ...
  const ButtunHistoriqueCommande = document.querySelector(
    ".buttun_historique_de_commande"
  );
  ButtunHistoriqueCommande.classList.remove("active");

  const ButtunMarquesFavorites = document.querySelector(
    ".buttun_marques_favorites"
  );
