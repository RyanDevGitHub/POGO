...
const div_container_profil_client = document.querySelector(".container-info-client");
...
const input = document.getElementsByName("search-bar");
const formulaire_recherche_information_clients = document.querySelector(".form-search");

formulaire_recherche_information_clients.addEventListener("keyup",() => {
    console.log("echo");
    form = document.forms["form-search"]["search-bar"];
    ...
    if(form.value.length > 0){
        ...
        xhr.open("POST","./back/auto_sugest.php",true);
        ...
        xhr.onreadystatechange = function(){
            if(this.readyState == 4 && this.status === 200 ){
                ...
                for(i =0 ;i<response.length;i++){
                    ...
                    miA.setAttribute('onclick', 'add_search_in_input("'+mipseudo+'")');
...
function new_password(){
    var xhr = new XMLHttpRequest();
    xhr.open("POST","./back/reinit_password.php",true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.onreadystatechange = function(){
        if(this.readyState == 4 && this.status === 200 ){
