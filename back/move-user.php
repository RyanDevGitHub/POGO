...
$_GET['name'] = utf8_decode($_GET['name']);
$_GET['firstname'] = utf8_decode($_GET['firstname']);
...
if ($firstnameIsOk === true) {
    $_GET['firstname'] = utf8_encode($_GET['firstname']);
    include('../database/db.php');
...
if (isset($_GET['note']) && $_GET['note'] == "paiement") {
    ...
    header("Location: ../profil.php?modify=on&note=paiement");
} else if ($_GET['mode'] === 'admin') {
    header("Location: ../profil-admin.php?page=MonProfil&modify=on");
} else {
    header("Location: ../profil.php?modify=on");
}
